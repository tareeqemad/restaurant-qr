...
class StaffMealDebtControlTest extends TestCase
{
    ...
    public function test_block_policy_throws_when_charge_would_breach_ceiling(): void
    {
        ...
        $this->charge($this->employee, qty: 9);
    ...
    public function test_block_policy_allows_charges_inside_ceiling(): void
    {
        ...
        $this->assertSame(500.0, (float) $charge->amount,
            'Even on block policy, in-budget charges go through cleanly.');
    }
    ...
    public function test_threshold_alerts_fire_at_80_100_and_120_percent(): void
    {
        ...
        $alerts = ActivityLog::where('event', 'staff_meal.threshold')->where('subject_id', $this->employee->id)->get();
    ...
    public function test_close_month_batch_settles_open_charges_and_links_them_to_closure(): void
    {
        ...
        $this->charge($this->employee, qty: 3);   // 300
        $other = $this->staff('other_emp', 'waiter');
        $other->update(['monthly_meal_allowance' => 400]);
        $orderOther = $this->placeStaffOrder($other, qty: 2);
    ...
    {
        $this->actingAs($this->manager);
        $this->charge($this->employee, qty: 2);   // 200
    ...
    {
        $this->actingAs($this->manager);
        $this->charge($this->employee, qty: 3);   // 300, under 500 allowance
        ...
        $this->assertEqualsWithDelta(0.0, $row['overflow'], 0.001, 'Under allowance, no overflow on the sheet.');
    }
    ...
    {
        $this->actingAs($this->manager);
        $charge = $this->charge($this->employee, qty: 1);   // 100
    ...
    {
        $this->actingAs($this->manager);
        $charge = $this->charge($this->employee, qty: 1);
    ...
    public function test_gift_order_records_a_charge_that_is_already_settled_and_does_not_touch_the_tab(): void
    {
        ...
        $this->assertSame(200.0, (float) $charge->amount, 'Gift keeps the order amount on the row for reporting.');
        ...
        // Accounting: charge + gift-settle posts both legs.
        $settle = JournalEntry::where('event_type', 'staff_meal_settled_gift')
            ->where('source_id', $charge->id)
            ->with('lines.account')
            ->first();
        $this->assertNotNull($settle, 'Gift settlement MUST post its own journal entry.');
        $debit = $settle->lines->firstWhere(fn ($l) => $l->debit > 0);
        $this->assertSame('5060', $debit->account->code,
            'Gift posts to the dedicated 5060 staff-gifts-and-perks expense account.');
    }
    ...
    {
        $this->actingAs($this->manager);
        $charge = $this->charge($this->employee, qty: 3);   // 300
        $this->assertSame(300.0, $this->employee->fresh()->staffMealOutstanding());
    ...
    public function test_full_cycle_keeps_1110_receivable_balanced_to_zero(): void
    {
        ...
        $lines = \App\Models\JournalLine::join('accounts', 'accounts.id', '=', 'journal_lines.account_id')
    ...
    public function test_monthly_summary_reports_gifted_amount(): void
    {
        ...
        $this->assertSame(100.0, (float) $summary['used'], 'Only the regular 100 counts as used.');
