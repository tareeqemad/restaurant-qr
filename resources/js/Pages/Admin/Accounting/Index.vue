...
<template>
    ...
    <PageHeader title="مركز المحاسبة" icon="bi-calculator-fill" subtitle="راجع ما سجله النظام، صحّح باستقلالية، ثم أقفل الفترة بثقة">
        <template #actions>
            <a v-if="urls.manualEntry" :href="urls.manualEntry" class="btn btn-primary"><i class="bi bi-plus-lg"></i> قيد يدوي</a>
        </template>
    </PageHeader>
    ...
    <section class="acc-hero">
        ...
        <a :href="urls.guide">افهم مسار كل قيد <i class="bi bi-arrow-left"></i></a>
    </section>
    ...
    <div class="acc-home-grid">
        <div class="acc-home-main">
            <AccountingPanel title="ماذا تريد أن تنجز الآن؟" description="ابدأ بالعمل المطلوب مباشرة؛ بقية الأدوات موجودة في الشريط العلوي." icon="bi-check2-square">
                <div class="task-grid">
                    <a v-for="task in tasks" :key="task.label" :href="task.url" :data-tone="task.tone">
            ...
            <AccountingPanel title="آخر القيود" description="آخر ما وصل إلى الدفتر من جميع العمليات." icon="bi-clock-history" compact>
                <div v-if="latestEntries.length" class="entries-list">
                    <article v-for="entry in latestEntries" :key="entry.id">
                        ...
                        <a v-if="entry.adjustUrl" :href="entry.adjustUrl" title="تصحيح القيد"><i class="bi bi-arrow-counterclockwise"></i></a>
                    </article>
                </div>
                ...
                <template #footer><a :href="urls.journal" class="panel-link">عرض دفتر القيود كاملاً <i class="bi bi-arrow-left"></i></a></template>
            </AccountingPanel>
        </div>
        ...
        <aside class="acc-home-side">
            ...
            <AccountingPanel title="جاهزية التأسيس" description="ثلاث نقاط تكفي لبدء محاسبة سليمة." icon="bi-shield-check" tone="blue">
                <div class="setup-checks">
                    <a :href="urls.accounts"><i class="bi bi-check-circle-fill"></i><span><strong>شجرة الحسابات</strong><small>{{ health.accounts }} حساباً فعالاً</small></span><i class="bi bi-chevron-left"></i></a>
                    <a v-if="urls.openingBalances" :href="urls.openingBalances"><i class="bi" :class="health.journalEntries ? 'bi-check-circle-fill' : 'bi-circle'"></i><span><strong>الأرصدة الافتتاحية</strong><small>{{ health.journalEntries ? 'الدفتر يحتوي حركات' : 'أدخل أرصدة البداية عند الحاجة' }}</small></span><i class="bi bi-chevron-left"></i></a>
                    <a v-if="urls.fiscalYears" :href="urls.fiscalYears"><i class="bi" :class="health.currentPeriod ? 'bi-check-circle-fill' : 'bi-circle'"></i><span><strong>السنة والفترة</strong><small>{{ health.currentPeriod ? health.currentPeriod.name : 'غير محددة بعد' }}</small></span><i class="bi bi-chevron-left"></i></a>
                </div>
            </AccountingPanel>
        </aside>
    </div>
...
@media(max-width:1100px){.health-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.acc-home-grid{grid-template-columns:1fr}.acc-home-side{grid-template-columns:repeat(2,minmax(0,1fr))}.acc-home-side>:last-child{align-self:start}}
@media(max-width:680px){.acc-hero{align-items:flex-start;flex-wrap:wrap}.acc-hero>a{width:100%;padding-inline-start:57px}.health-grid,.task-grid,.acc-home-side,.impact-grid{grid-template-columns:1fr}.entries-list article{grid-template-columns:34px minmax(0,1fr) auto}.entries-list time{display:none}.entries-list bdi{grid-column:2}.entries-list article>a{grid-column:3;grid-row:1/3}.impact-toggle small{display:none}}
</style>
