    ...
    <nav v-if="shown" class="inv-workspace" aria-label="مركز المخزون والمشتريات">
        <div class="inv-workspace__groups" role="tablist">
            <button v-for="group in groups" :key="group.id" type="button"
                    :class="{ active: shown.id === group.id, current: activeGroup === group.id }"
                    @click="openGroup = group.id">
                <i class="bi" :class="group.icon"></i><span>{{ group.label }}</span>
                <small>{{ group.items.length }}</small>
            </button>
        </div>
        <div class="inv-workspace__links">
            <a v-for="item in shown.items" :key="item.key" :href="item.href"
...
.inv-workspace__groups button.active{color:#176b39;background:#eef7f1}.inv-workspace__groups button.current{box-shadow:inset 0 -2px #17713c}
