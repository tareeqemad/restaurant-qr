...
{{-- ═══════════════════════════════════════════════════════════════════
     ...
     Rendered inline beneath the user-edit form so admins can
     over-grant/revoke from day 1.  The actual markup lives in the
     reusable partial admin.permissions._tree, which is ALSO used by
     the standalone /admin/permissions page.

     Single source of truth → both pages stay visually + behaviourally
     identical, and the file shrinks from 850 lines to a single include.
═══════════════════════════════════════════════════════════════════ --}}
@if(auth()->user()?->hasPermission('roles.update') && ! ($isOwnerLevel ?? false))
    <div class="mt-4">
        @include('admin.permissions._tree')
    </div>
