...
<div class="track-wrap">
    ...
    @else
        ...
        <div class="live-pill" id="liveStatus">
            <span class="live-dot"></span>
            <span class="live-label">مباشر</span>
            <span class="last-updated" id="lastUpdated">الآن</span>
        </div>
        ...
        {{-- Add more button (only if there are active orders) --}}
        @if($active->count())
            <a href="{{ route('customer.menu') }}" class="btn-track-primary" style="background: linear-gradient(135deg, var(--accent) 0%, var(--accent-dark) 100%); box-shadow: 0 8px 24px rgba(184,135,42,.4); color: var(--brand-dark);">
...
(function () {
    ...
    // If Echo (websocket) is available, we're LIVE. Otherwise fall back to polling.
    const hasEcho = () => typeof window.Echo !== 'undefined' && window.Echo;

    function setOnline() {
        pill.classList.remove('offline', 'refreshing');
    ...
    function setRefreshing() {
        ...
        label.textContent = 'يتم التحديث...';
    }

    // Update "last updated" every 10s
    function updateLastSeen() {
        if (!lastUpd) return;
        const seconds = Math.floor((Date.now() - loadedAt) / 1000);
        if (seconds < 5)       lastUpd.textContent = 'الآن';
        else if (seconds < 60) lastUpd.textContent = 'منذ ' + seconds + ' ث';
        else                   lastUpd.textContent = 'منذ ' + Math.floor(seconds / 60) + ' د';
    }
    updateLastSeen();
    setInterval(updateLastSeen, 10000);

    // Check connectivity / websocket state
    function checkStatus() {
        if (!navigator.onLine) return setOffline();
        if (hasEcho())         return setOnline();
        // No websocket yet — keep polling mode indicator subtle
        setOnline();
    }
    checkStatus();
    ...
    // Auto-refresh fallback every 30s in case websocket down.
    // Show "refreshing" pulse just before reload for better UX.
    setTimeout(() => {
        setRefreshing();
